...
class SemanticCache:
    ...
    def __init__(
        ...
        logger.info(
            f"Cache inicializado: {len(self.texts)} itens, dimensão {self.dimension}"
        )
    ...
    def _determine_dimension(self, provided_dimension: Optional[int]) -> int:
        ...
        if provided_dimension is not None:
            if not (self.MIN_DIMENSION <= provided_dimension <= self.MAX_DIMENSION):
                raise ValueError(
                    f"Dimensão deve estar entre {self.MIN_DIMENSION} e {self.MAX_DIMENSION}"
                )
        ...
        try:
            ...
            if not (self.MIN_DIMENSION <= detected_dim <= self.MAX_DIMENSION):
                raise ValueError(
                    f"Dimensão detectada ({detected_dim}) fora dos limites seguros"
                )
    ...
    def _get_embedding(self, text: str, retry_count: int = 2) -> np.ndarray:
        ...
        for attempt in range(retry_count + 1):
            try:
                ...
                logger.debug(
                    f"Raw embedding type: {type(raw_embedding)}, length: {len(raw_embedding)}"
                )
                ...
                if vec.ndim == 2 and vec.shape[0] == 1:
                    ...
                    raise ValueError(
                        f"Unexpected embedding shape: {vec.shape}. Expected 1D array after flattening."
                    )

                if vec.shape[0] != self.dimension:
                    raise ValueError(
                        f"Embedding dimension mismatch! Expected {self.dimension}, got {vec.shape[0]} after flattening."
                    )
                ...
                if norm == 0:
                    if attempt < retry_count:
                        logger.warning(
                            f"Embedding com norma zero (tentativa {attempt + 1}), tentando novamente..."
                        )
                        ...
                        raise ValueError(
                            f"Embedding com norma zero após {retry_count + 1} tentativas. Texto: '{text[:50]}...'"
                        )
                ...
                if attempt < retry_count:
                    ...
                    logger.error(
                        f"Falha definitiva no embedding após {retry_count + 1} tentativas: {e}"
                    )
                    raise
    ...
    def _should_auto_save(self) -> bool:
        ...
        return (
            self.unsaved_changes >= self.auto_save_interval
    ...
    def search(
        self, query_text: str, k: int = 1, min_score: float = 0.75
    ) -> List[Dict[str, Any]]:
        """
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        with self._lock:
            ...
            return {
                ...
                "metadata_size_mb": (
                    self.metadata_path.stat().st_size / (1024 * 1024)
                    if self.metadata_path.exists()
                    else 0
                ),
