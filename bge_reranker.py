...
try:
    ...
    logger.warning(
        "A biblioteca \'rerankers\' ou suas dependências (e.g., transformers, torch) não estão instaladas. O reranking será desabilitado."
    )
...
class BGEReranker:
    def __init__(self, model_name: str = "BAAI/bge-reranker-base"):
        ...
        if RERANKER_AVAILABLE:
            try:
                ...
                logger.error(
                    f"Erro ao inicializar BGE Reranker \'{model_name}\': {e}. O reranking será desabilitado."
                )
                self.reranker = None
    ...
    def rerank(
        ...
        if not self.reranker:
            logger.warning(
                "Reranker não está disponível. Retornando documentos originais (limitado por top_n)."
            )
        ...
        try:
            ...
            logger.error(
                f"Erro durante o reranking: {e}. Retornando documentos originais (limitado por top_n)."
            )
            # Fallback: return top_n documents without reranking
            return [(doc, 0.0) for doc in documents[:top_n]]
