...
from ..models import Node, ValueScore, RunConfig
...
class HybridEvaluator:
    def __init__(self, config: RunConfig):
        self.config = config
        self.llm = get_chat_llm(model="gemini-2.5-flash", temperature=self.config.value_temp)
        self.structured_llm = self.llm.with_structured_output(LLMValueScore)


    def _llm_evaluate(self, task_instruction: str, candidate_text: str, history_text: str) -> ValueScore:
        ...
        try:
            ...
            return ValueScore(progress=0.0, promise=0.0, confidence=0.0, justification=f"Parsing error: {e}")
    ...
    def evaluate(self, node: Node, task_instruction: str, all_nodes: Dict[str, Node]) -> Node:
        ...
        if heuristic_score is not None:
            ...
            node.raw_scores = ValueScore(progress=heuristic_score, promise=heuristic_score, confidence=heuristic_score, justification="Heuristic low score")
            return node
