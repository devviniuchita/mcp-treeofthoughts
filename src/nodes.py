...
def propose_thoughts(state: GraphState) -> GraphState:
    ...
    for node_id in nodes_to_expand_ids:
        ...
        cached_results = state.semantic_cache.search(
            cache_key, k=1, min_score=0.75
        )  # Using min_score for cosine similarity

        candidates_text = []
        if cached_results:
            print(f"[CACHE HIT] Propose for node {node.id} from cache.")
            if "candidates" in cached_results[0]["metadata"]:
                candidates_text = json.loads(
                    cached_results[0]["metadata"]["candidates"]
                )
            else:
                print(
                    f"[CACHE MISS] \'candidates\' key not found in cache metadata for node {node.id}. Proceeding with LLM call."
                )
        ...
        if not candidates_text:  # If no candidates from cache, or cache miss, call LLM
            ...
            try:
                ...
                print(
                    f"[ERROR] JSON decode error in propose_thoughts: {e}. Raw output: {raw_output}"
                )
                # Fallback: split by lines if JSON parsing fails
                candidates_text = [
                    t.strip() for t in raw_output.split('\n') if t.strip()
                ][: state.config.branching_factor]
            except Exception as e:
                print(
                    f"[ERROR] Unexpected error in propose_thoughts: {e}. Raw output: {raw_output}"
                )
                candidates_text = []

            # Add to semantic cache
            state.semantic_cache.add(
                cache_key, metadata={"candidates": json.dumps(candidates_text)}
            )

        for candidate_text in candidates_text[: state.config.branching_factor]:
            child_node = Node(
                text=candidate_text, parent_id=node.id, depth=node.depth + 1
            )
    ...
    state.frontier = [
        n.id for n in new_nodes_generated
    ]  # Frontier now contains newly generated nodes to be evaluated
    return state
        ...
        if cached_results:
            ...
            if "score" in metadata and "raw_scores" in metadata:
                try:
                    ...
                    print(
                        f"[CACHE MISS] Error parsing cached evaluation for node {node.id}: {e}. Proceeding with LLM call."
                    )
            else:
                print(
                    f"[CACHE MISS] Missing 'score' or 'raw_scores' in cached metadata for node {node.id}. Proceeding with LLM call."
                )

        if evaluated_node is None:  # If not from cache (or cache was invalid), call LLM
            evaluated_node = evaluator.evaluate(
                node, state.task.instruction, state.nodes
            )
            ...
            state.semantic_cache.add(
                ...
                metadata={
                    ...
                    "raw_scores": (
                        evaluated_node.raw_scores.model_dump_json()
                        if isinstance(evaluated_node.raw_scores, BaseModel)
                        else json.dumps(evaluated_node.raw_scores)
                    ),
                },
            )
        evaluated_nodes.append(
            evaluated_node
        )  # Append the (potentially updated) evaluated_node

    # Update frontier to contain the IDs of the evaluated nodes, sorted by score
    evaluated_nodes.sort(
        key=lambda n: n.score if n.score is not None else -1, reverse=True
    )
...
def select_and_prune(state: GraphState) -> GraphState:
    ...
    state = strategy.update_frontier(
        state, [state.nodes[node_id] for node_id in state.frontier]
    )
    ...
    if best_node_from_strategy:
        ...
        if current_best_overall is None or (
            best_node_from_strategy.score is not None
            and best_node_from_strategy.score
            > (
                current_best_overall.score
                if current_best_overall.score is not None
                else -1
            )
        ):
    ...
    print(
        f"[SELECT] Frontier after selection/pruning: {len(state.frontier)} nodes. Best node score: {state.nodes[state.best_node_id].score if state.best_node_id else 'N/A'}"
    )
    return state
...
def check_stop_condition(state: GraphState) -> str:
    ...
    if (time.time() - state.start_time) >= state.config.stop_conditions.get(
        "max_time_seconds", 30
    ):
    ...
    if (
        current_best_node
        and current_best_node.score is not None
        and current_best_node.score >= 9.5
    ):
    ...
    if all(
        state.nodes[node_id].depth >= state.config.max_depth
        for node_id in state.frontier
    ):
    ...
    print(
        f"[CONTINUE] Current best score: {current_best_node.score if current_best_node else 'N/A'}. Nodes expanded: {state.nodes_expanded}. Frontier size: {len(state.frontier)}"
    )
    return "continue"
...
def finalize_solution(state: GraphState) -> GraphState:
    ...
    if not best_node:
        state.final_answer = (
            "No solution found."  # Fallback if no best node is identified
        )
        print("[FINALIZE] No best node found.")
        return state

    print(
        f"[FINALIZE] Finalizing solution from best node: {best_node.id} with score {best_node.score}"
    )
...
def rerank_thoughts(state: GraphState) -> GraphState:
    ...
    reranked_results = reranker_instance.rerank(
        query=query, documents=thoughts_to_rerank, top_n=state.config.reranker_top_n
    )

    # Create a mapping from original thought text to its node_id
    thought_text_to_id = {
        state.nodes[node_id].text: node_id for node_id in state.frontier
    }
    ...
    print(
        f"[RERANK] Frontier after reranking: {len(state.frontier)} nodes (top {state.config.reranker_top_n})."
    )
