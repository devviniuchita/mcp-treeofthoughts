...
async def run_tot_process(req: RunRequest, background_tasks: BackgroundTasks):
    ...
    if run_id in active_runs:
        raise HTTPException(status_code=409, detail=f"Run with ID {run_id} already exists.")
    ...
    # Store initial state
    active_runs[run_id] = {"status": "running", "state": initial_state.model_dump(), "result": None}
        ...
        try:
            ...
            if isinstance(raw_final_state, GraphState):
                ...
                try:
                    ...
                    print(f"[WARNING] Could not parse raw_final_state into GraphState: {e}. Raw state: {raw_final_state}")
            ...
            final_answer_text = final_state.final_answer if isinstance(final_state, GraphState) else final_state.get("final_answer", "N/A")
...
async def get_run_status(run_id: str):
    ...
    return {"run_id": run_id, "status": run_data["status"], "metrics": run_data["result"].get("metrics") if run_data["result"] else None}
...
async def get_run_trace(run_id: str):
    ...
    if run_data["status"] == "running":
        return {"status": "running", "message": "Trace not available until run completes.", "current_state": run_data["state"]}
...
async def stop_run(run_id: str):
    ...
    if active_runs[run_id]["status"] == "running":
        active_runs[run_id]["status"] = "cancelled" # Mark as cancelled, but the background task might still run to completion
        return {"run_id": run_id, "status": "cancellation_requested", "message": "Cancellation mechanism not fully implemented for LangGraph background tasks. Task might complete."}
    return {"run_id": run_id, "status": active_runs[run_id]["status"], "message": "Run is not active or already stopped."}
