...
class BestFirstSearch(SearchStrategy):
    ...
    def select_nodes_for_expansion(self, state: GraphState) -> List[str]:
        ...
        best_node_id = max(
            state.frontier,
            key=lambda node_id: (
                state.nodes[node_id].score
                if state.nodes[node_id].score is not None
                else -1
            ),
        )
        return [best_node_id]
    ...
    def update_frontier(self, state: GraphState, new_nodes: List[Node]) -> GraphState:
        ...
        nodes_to_keep = [
            ...
            if node_id
            not in [n.parent_id for n in new_nodes if n.parent_id is not None]
        ]
        ...
        for node in new_nodes:
            ...
            ):  # Apenas nós avaliados podem ser adicionados à fronteira
